...
To derive an expression for $S_{\rm V}(f)$ we consider the white noise regime only, meaning we limit ourselves to higher frequencies to avoid any significant influence of 1/\textit{f}-noise. Now, we need to distinguish between two noise currents, one flowing across the SQUID and the other circulating within the SQUID loop. The former sees the total resistance of the SQUID which corresponds to $\frac{R_{\rm s}}{2}$ for $I\gg I_{\rm b,max}$ and to the dynamic resistance $R_{\rm dyn}=\frac{\del V}{\del I}$ for operation at the working point. The circulating current, however, sees two resistances in series, i.e. 2$R_{\rm s}$. By taking into account noise inducing resonances due to $\beta_{\rm c}\approx 1$ and $\beta_{\rm L}\approx 1$, both noise currents can be used to obtain the voltage noise
...
\gl{
\epsilon(f) = 16k_{\rm B}T\sqrt{L_{\rm s}C}
}{}

\gl{
S_{\rm \Phi} = S_{\rm \Phi,w} + \frac{S_{\rm \Phi, 1/f}(\qty{1}{\hertz})}{f^\alpha}
}{}
