...
= \Phi_{\mathrm{e}}-\frac{1}{2} \beta_L \Phi_0 \sin \left(\pi \frac{\Phi}{\Phi_0}\right) \cos \left(\varphi_1+\pi \frac{\Phi}{\Phi_0}\right) \ \ .
}{}

Here, we introduced the dimensionless screening parameter $\beta_{\rm L}=\frac{2L_{\rm s}I_{\rm c}}{\unit{\fq}}$, which relates the maximum possible flux $\Phi_{\rm cir}^{\rm max} = L_{\rm s}I_{\rm cir}^{\rm max} = L_{\rm s}I_{\rm c}$ produced by screening currents to $\frac{\unit{\fq}}{2}$. This quantity describes the influence the screening currents have on the total flux $\Phi$, which in turn affects $I_{\rm s}$ in equation \ref{IsGeneral_SQUID0v}. We will now simplify the expression above by considering the limiting case for $I_{\rm s}\ll 2I_{\rm c}$
