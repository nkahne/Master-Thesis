...
{../Figures/fraunhofer}
{0cm}
{Normalized flux-dependent maximum Josephson current $I_{\rm s}^{\rm m}(\Phi)$ showing a Fraunhofer pattern. It modulates with the flux quantum $\unit{\fq}$, peaking at $\Phi=\frac{(n+2)\unit{\fq}}{2}$. For $\Phi=n\unit{\fq}$ the total net current is zero.} 
{fig:fraunhofer}
